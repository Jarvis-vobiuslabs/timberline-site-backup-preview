...
public function parse_return($content)
{
	...
	$customerPayProfileId = $this->substring_between($content,'<customerPaymentProfileIdList>','</customerPaymentProfileIdList>');
	$customerPayProfileId2 = $this->substring_between($customerPayProfileId,'<numericString>','</numericString>');
	return array ($resultCode, $code, $text, $customerProfileId, $customerPayProfileId2); 
}
...
class AuthnetARB {
	...
	*/
	...
	private $test= false; // allows us to easily switch back and forth between our test account and our live account.
