<?php 
	...
	if(($_POST["submit"])) {
		...
		switch($_POST["service"]) {
			...
			$ARBresult = setup_ARB($_SERVER["PHP_SELF"], "Lawncare Subscription", $_POST["lcb-frequency"], "9999", false, $_POST["calendar"]);
			...
			submit_payment($_SERVER["PHP_SELF"]);
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="reset-order.css" />
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				$("#calendar").datepicker({ altField: '#calendar'});
...
<div id="container">
    ...
    <div class="clearer"><!-- <?php if($ARBresult) echo $ARBresult; else echo "false" ?> --></div>
    ...
    <div id="main">
		...
		<div id="signUp">
			...
			<?php			
				...
				switch($_GET["response"]) {
					...
					<div class="accepted">
						...
						<p style="margin-top: 2em;"><a href="index.html" style="background: #bfbfbf; color: #000; padding: 10px; text-decoration: none; font-size: 125%; margin-top: 1em;">Return to Homepage</a></p>
					...
					if($_SESSION['error']) { ?>
						...
						if($_SESSION['error']) echo "<!-- still set -->";
			...
			<form action="<?=$_SERVER['PHP_SELF']?>" method="post" name="paymentForm">
				...
				<table>
		            ...
		            <tr>
						...
						<td colspan="2">
							...
							<p class="center"><em>Please fill out the complete form &mdash; incomplete forms will not be processed</em></p>
		            ...
		            <tr>
		                ...
		                <td colspan="2">
							...
							<span id="siteseal"><script type="text/javascript" src="https://seal.godaddy.com/getSeal?sealID=2E9hKVVWnKHHhfblTPIM6p7Dqklc3ws00Hv6SFxD7U2f3SU7cv"></script><br />
		            ...
		            <tr><td colspan="2"><h4>Service Address</h4></td></tr>
			            ...
			            <tr>
			                ...
			                <p>Please select your service area</p>

							<select name="zip">

		                    	<option></option>

								<option value="75248"<? if($_SESSION['zip'] == "75248") echo " selected"; ?>>75248</option>

								<option value="75254"<? if($_SESSION['zip'] == "75254") echo " selected"; ?>>75254</option>

								<option value="75240"<? if($_SESSION['zip'] == "75240") echo " selected"; ?>>75240</option>

								<option value="75230"<? if($_SESSION['zip'] == "75230") echo " selected"; ?>>75230</option>

								<option value="75243"<? if($_SESSION['zip'] == "75243") echo " selected"; ?>>75243</option>

								<option value="75225"<? if($_SESSION['zip'] == "75225") echo " selected"; ?>>75225</option>

								<option value="75231"<? if($_SESSION['zip'] == "75231") echo " selected"; ?>>75231</option>

								<option value="75238"<? if($_SESSION['zip'] == "75238") echo " selected"; ?>>75238</option>

								<option value="75206"<? if($_SESSION['zip'] == "75206") echo " selected"; ?>>75206</option>

								<option value="75214"<? if($_SESSION['zip'] == "75214") echo " selected"; ?>>75214</option>

								<option value="75218"<? if($_SESSION['zip'] == "75218") echo " selected"; ?>>75218</option>

								<option value="75228"<? if($_SESSION['zip'] == "75228") echo " selected"; ?>>75228</option>

								<option value="75204"<? if($_SESSION['zip'] == "75204") echo " selected"; ?>>75204</option>

								<option value="75219"<? if($_SESSION['zip'] == "75219") echo " selected"; ?>>75219</option>

								<option value="75205"<? if($_SESSION['zip'] == "75205") echo " selected"; ?>>75205</option>

								<option value="75041"<? if($_SESSION['zip'] == "75041") echo " selected"; ?>>75041</option>

								<option value="75042"<? if($_SESSION['zip'] == "75042") echo " selected"; ?>>75042</option>

								<option value="75081"<? if($_SESSION['zip'] == "75081") echo " selected"; ?>>75081</option>

								<option value="75080"<? if($_SESSION['zip'] == "75080") echo " selected"; ?>>75080</option>

								<option value="75150"<? if($_SESSION['zip'] == "75150") echo " selected"; ?>>75150</option>

								<option value="75209"<? if($_SESSION['zip'] == "75209") echo " selected"; ?>>75209</option>

								<option value="75220"<? if($_SESSION['zip'] == "75220") echo " selected"; ?>>75220</option>

								<option value="75229"<? if($_SESSION['zip'] == "75229") echo " selected"; ?>>75229</option>

								<option value="75244"<? if($_SESSION['zip'] == "75244") echo " selected"; ?>>75244</option>
		            ...
		            <tr><td colspan="2">
		            	...
		            	<tr>
		            		...
		            		<td>
		            			...
		            			<ul>
									...
									<li><input id="list-services-lcb" type="radio" name="service" value="lawncare-big"<? if($_SESSION['service'] == "lawncare-big") echo " checked=\"checked\""; ?> onclick="showService('service-details-lcb');" />Lawn Service</li>

									<li><input id="list-services-ls" type="radio" name="service" value="landscaping"<? if($_SESSION['service'] == "landscaping") echo " checked=\"checked\""; ?> onclick="showService('service-details-landscape');" />Landscaping</li>
							...
							<td>
								...
								<div id="service-details">
									...
									<div id="service-details-lcb">
										...
										<p><input type="checkbox" name="promo-offer" value="<? if($_SESSION['promo-offer']=="promo-offer") echo " checked=\"checked\""; ?>" />Check for promotional offer</p>
										...
										<p>
											...
											<label for="lcb-lotsize">Lot Size:</label><input type="text" name="lcb-lotsize" value="<? echo $_SESSION['lcb-lotsize']; ?>" />sq. ft.

											<span class="instruct" style="text-align: left; margin-left: 70px">(if applicable)</span>

										</p>

										<p>

											<label for="lcb-estimate">Estimate:</label><input type="text" name="lcb-estimate" value="<? echo $_SESSION['lcb-estimate']; ?>" />
									...
									<div id="service-details-landscape">
										...
										<p><label for="ls-estimate">Estimate:</label><input type="text" name="ls-estimate" value="<? echo $_SESSION['ls-estimate']; ?>" /></p>

										<p><label for="ls-price">Price:</label>$<input type="text" name="ls-price" value="<? echo $_SESSION['ls-price']; ?>" /> (e.g. 25)</p>
						...
						<tr>
			                ...
			                <td><textarea name="notes" rows="2" cols="23"><? echo $_SESSION['notes']; ?></textarea></td>
		            ...
		            <tr><td colspan="2"><h4>Credit Card Information</h4></td></tr>
						...
						<tr>
							...
							<td><input type="checkbox" name="billingaddrsame" onclick="set_billing(this)" /> <span style="text-align: left; font-weight: bold;">(check this box if billing address is the same as service address above)</span></td>
					...
					<tr>
						...
						<td colspan="2" class="center">
							...
							<p style="margin-top: 2em;"><a href="index.html" style="background: #bfbfbf; color: #000; padding: 10px; text-decoration: none; font-size: 125%;">Return to Homepage</a></p>
