...
require_once("authnet2.class.php");
...
function submit_payment($redirect) {
	...
	$payment->setParameter("x_address", $streetAddress); 
	$payment->setParameter("x_city", $theCity);
	$payment->setParameter("x_state", $theState);
	...
	if ($payment->isApproved()) { 
		...
		// Possible things to do now: setup ARB, add to DB, send emails, etc
			...
			header("Location: $redirect?response=success&transaction=$transID");
	} 
...
function setup_ARB($redirect, $subscriptionName="Subscription", $frequency="monthly", $until="9999", $verify=false, $start_date=false) {
	...
	if ($arb->isSuccessful()) { 
		...
		header("Location: $redirect?response=success&transaction=$subscriber_id");	
	}
	else if ($arb->isError()) {
